...
def makeStar(rec):
    for i in range(len(rec)):
        if rec[i].y == -1:
            rec[i].x = random.randint(100, SCREEN_WIDTH - 50)  # 장애물 위치 랜덤 배치
            rec[i].y = random.randint(50, SCREEN_HEIGHT - 50)
#########################################################
def moveObstacles(current):
    ...
    for i in range(len(current)):
        ...
        if current[i].x < -current[i].width:
            current[i].x = SCREEN_WIDTH + random.randint(50, 200)  # 화면 밖으로 나가면 다시 오른쪽에서 나타나게 설정
...
#########################################################
##4. 장애물
star = [pygame.image.load("star.png") for i in range(5)]
